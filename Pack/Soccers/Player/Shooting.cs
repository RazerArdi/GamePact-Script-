        ...
        if (GameManager.Instance.gameActive)
        {
            if (_player.user)
            {
                if (_player.hasPossession)
                {
                    if (_player.canShoot)
                    {
                        if (/*Input.GetButtonDown("Shoot")*/ ShootButtonHandler.Instance.isButtonDown)
                        {
                            _shotTimer = 0;

                            ShootButtonHandler.Instance.isButtonDown = false;
                        }
                        
                        if (/*Input.GetButton("Shoot")*/ ShootButtonHandler.Instance.isButtonHold)
                        {
                            AimTarget();
                        ...
                        if (/*(Input.GetButtonUp("Shoot")*/ShootButtonHandler.Instance.isButtonUp || _shotTimer > 0.75f)
                        {
                            ShootBall();
                            ...
                            ShootButtonHandler.Instance.isButtonUp = false;
                        }
                    }   
                }
            }
