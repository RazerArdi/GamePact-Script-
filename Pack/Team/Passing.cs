...
public class Passing : MonoBehaviour
{
    ...
    private void PassHandler()
    {
        if (GameManager.Instance.gameActive)
        {
            ...
            {
                if (/*Input.GetButtonDown("Pass")*/ThrowButtonHandler.Instance.isButtonDown)
                {
                    _passBegun = true;
                    ThrowButtonHandler.Instance.isButtonDown = false;
                }
                    

                if (/*Input.GetButton("Pass")*/ThrowButtonHandler.Instance.isButtonHold)
                {
                ...
                if (_passBegun)
                {
                    if (/*Input.GetButtonUp("Pass")*/ ThrowButtonHandler.Instance.isButtonUp || _passTimer > 1)
                    {
                        ThrowButtonHandler.Instance.isButtonUp = false;
